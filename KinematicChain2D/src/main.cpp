...
std::shared_ptr<RobotSimulation> CreateSimulation(
...
std::shared_ptr<RobotSimulation> CreateSimulation(
        std::shared_ptr<ifx::SceneContainer> scene){
    return std::shared_ptr<RobotSimulation>(new RobotSimulation(scene));
}
...
int main() {
    ...
    game_object3->moveTo(glm::vec3(0, 0, 7));
    game_object3->rotateTo(glm::vec3(-90, 0, 0));

    //game->scene()->Add(game_object1);
    game->scene()->Add(game_object2);
    game->scene()->Add(game_object3);

    auto hodograph_simulation = CreateSimulation(game->scene());
